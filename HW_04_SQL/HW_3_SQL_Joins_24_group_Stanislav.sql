...
--у меня было так = WHERE employee_salary_3.employee_id >70;
...
--у меня было WHERE employee_salary_3.employee_id >70 AND monthly_salary <2000;
...
--5. Найти всех работников кому не начислена ЗП. (У Джеффа, самое оптимальное решение)
SELECT employee_name FROM employees
LEFT JOIN employee_salary_3
ON employees.id = employee_salary_3.employee_id
WHERE salary_id IS NULL;

--5. Найти всех работников кому не начислена ЗП. (НЕ МОЁ, это если salary_id >15!)
SELECT * FROM employees
JOIN employee_salary_3
ON employees.id = employee_salary_3.employee_id
LEFT JOIN salary_3
...
from roles_3 join roles_employee
on roles_3.id  = roles_employee.role_id 
where role_name like '% QA engineer';
